jQuery( document ).ready( function( $ ) {
	...
	function EegStripeConnect( stripe_instance_vars, translations ) {
		...
		this.connect_btn_listeners = function() {
			const $sandbox_mode_select = this.form.find( 'select[name*=stripe][name*=PMD_debug_mode]' );
			// Update connect button text depending on the PM sandbox mode.
			const stripe_connect_instance = this;
			...
			$( this.connect_btn_id ).on( 'click', function( event ) {
				...
				const button_container = $( this ).closest( 'tr' );
				const submitting_form = $( this ).parents( 'form' ).eq( 0 )[ 0 ];
				if ( button_container && submitting_form ) {
					...
					setTimeout( function() {
						...
						const win_loader = stripe_connect_instance.oauth_window.document.getElementById(
					...
					const debug_mode_selector = submitting_form.querySelector( 'select[name*=PMD_debug_mode]' );
			...
			this.hideNonApplicableInputs( this.form.parents( 'form' ).find( '.stripe-integration' ) );
			const _this = this;
			$( '.stripe-integration' ).change( function( event ) {
				_this.hideNonApplicableInputs( $( event.target ) );
			} );
		};

		this.hideNonApplicableInputs = function( target ) {
			const zipInputRow = target.parents( 'form' ).find( '.validate-zip' ).closest( 'tr' );

			if ( target.val() == 'checkout' ) {
				zipInputRow.css( 'display', 'table-row' );
			} else {
				zipInputRow.css( 'display', 'none' );
			}
		};
		...
		this.update_connect_button_text = function( caller, allow_alert ) {
			const submitting_form = caller.parents( 'form' ).eq( 0 )[ 0 ];
			if ( submitting_form ) {
				const stripe_connect_btn = $( submitting_form ).find(
					'a[id=' + this.connect_btn_id.substr( 1 ) + ']'
				)[ 0 ];
				if ( stripe_connect_btn ) {
					const btn_text_span = $( stripe_connect_btn ).find( 'span' )[ 0 ];
					const pm_debug_mode_enabled = caller[ 0 ].value;
					const disconnect_section = $( submitting_form ).find(
					...
					if ( allow_alert && disconnect_section && $( disconnect_section ).is( ':visible' ) ) {
						const test_connected = $( disconnect_section ).find(
		...
		this.update_disconnect_section = function( submitting_form ) {
			const pm_debug_mode = $( submitting_form ).find( 'select[name*=PMD_debug_mode]' )[ 0 ];
			const disconnect_section = $( submitting_form ).find(
				'tr[class="eeg-stripe-disconnect-section"]'
			)[ 0 ];
			if ( disconnect_section && pm_debug_mode ) {
				const test_connected_text = $( disconnect_section ).find(
					'strong[class="eeg-stripe-test-connected-txt"]'
				)[ 0 ];
		...
		this.oauth_send_request_success = function( response, request_action ) {
			const $ajax_spinner = $( '#espresso-ajax-loading' );
			if ( response === null ||
                response[ 'stripe_error' ] ||
                typeof response[ 'stripe_success' ] === 'undefined' ||
                response[ 'stripe_success' ] === null
			) {
				let stripe_error = this.translations.oauth_request_error;
		...
		this.oauth_send_request_error = function( response, error, description ) {
			let stripe_error = this.translations.error_response;
			if ( description ) {
	...
	const stripe_connect_objs = {};
	for ( const slug in ee_form_section_vars.stripe_connect ) {
