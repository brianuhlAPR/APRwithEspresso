 ...
 * Class BillingForm
 *
 * Form for displaying Stripe Checkout billing form (which is mostly just their pop-up window).
 *
...
class BillingForm extends EE_Billing_Info_Form
{
    ...
    public function __construct(EE_Payment_Method $payment_method, array $options_array = array())
    {
        if (! isset($options_array['transaction']) || ! $options_array['transaction'] instanceof EE_Transaction) {
            // Avoid a fatal error when someone pays using Elements, then the admin switches Stripe back to
            // Checkout. In this case, we think we have billing info for Stripe, and this form gets instantiated
            // WITHOUT a transaction. So don't blow up if there's no transaction, but we won't necessarily need a
            // billing form that's ready to process transactions. We just need to be able to construct it, then verify
            // there is no billing data that applies to it.
            $email = '';
            $amount = 0;
            $event_name = '';
            $transaction = null;
